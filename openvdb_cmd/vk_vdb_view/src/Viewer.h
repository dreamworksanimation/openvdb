...
enum class ViewerBackend {eOpenGL, eVulkan};
...
Viewer init(const std::string& progName, bool background, ViewerBackend aBackend);
...
public:
    ...
    void open(int width = DEFAULT_WIDTH, int height = DEFAULT_HEIGHT, uint32_t samples = 1);
...
private:
    friend Viewer init(const std::string&, bool, ViewerBackend);
    Viewer();
};
